...
\\
\textit{Autorzy:} &\textbf {Mateusz~Bieñkowski \newline
	Katarzyna~Go³êbiewska \newline
	Filip~Grajek \newline
	Sebastian~Sudra \newline
	£ukasz~Sznajder \newline
	Nikodem~Wiœniewski \newline 
...
Projekt realizowany w ramach konkursu \textit{Santander Customer Transaction Prediction}\cite{santanderkaggle}. Celem projektu jest opracowanie jak klasyfikatora, który na podstawie dostêpnych danych o konsumencie wska¿e, czy dokona on transakcji czy te¿ nie. 

\section{Opis danych}

Do dyspozycji uczestników konkursu organizator przygotowa³ dwa zanonimizowane zbiory danych: \textit{train.csv} oraz \textit{test.csv}. Oba zbiory danych posiadaj¹ po 200 tysiêcy wierszy. Ka¿dy wiersz posiada identyfikator \textit{ID\_code} oraz 200 anonimowych cech (\textit{var0, var1 ... var199}). Zbiór \textit{train.csv} posiada dodatkowo kolumnê \textit{target} bêd¹c¹ etykiet¹, przyjmuje ona wartoœci $0$ lub $1$. Dane z pliku \textit{train.csv} s³u¿¹ do trenowania i wstêpnej oceny jakoœci modelu. Plik \textit{test.csv} s³u¿y do oceny modelu przez organizatora konkursu, klasy dla tego zbioru nie s¹ publicznie znane. Zg³oszeniem do konkursu jest plik zestawiaj¹cy \textit{ID\_code} ka¿dego rekordu z pliku \textit{test.csv} z jego przewidywan¹ klas¹.


\subsection{Przetwarzanie i analiza danych}

Przetwarzanie i analiza danych s¹ czêsto uwa¿ane za najwa¿niejsze narzêdzia w osi¹ganiu ponadprzeciêtnych wyników w zagadnieniach zwi¹zanych ze sztuczn¹ inteligencj¹. Przetwarzanie danych nale¿y rozpocz¹æ od sprawdzenia ich integralnoœci. Przeszukanie wszystkich wierszy obu zbiorów nie wykaza³o ¿adnych brakuj¹cych pól. Kolejn¹ wa¿n¹ informacj¹ jest liczba przyk³adów w obu klasach. Im bardziej nierównomierne s¹ licznoœci zbiorów, tym trudniej stworzyæ model, który nie bêdzie siê przeucza³ w stronê wskazywania na liczniejsz¹ klasê. Dla takich problemów nie mo¿na równie¿ korzystaæ z miary dok³adnoœci jak¹ jest procent prawid³owych klasyfikacji. W takich przypadkach nale¿y u¿yæ bardziej zaawansowanych metod porównywania modeli klasyfikacyjnych, np. wskaŸnika Dice Coefficient. Zbiór treningowy jest podzielony na 20098 (10.049\%) konsumentów, którzy dokonali transakcji oraz 179902 (89.951\%) konsumentów, którzy nie dokonali transakcji (rysunek \ref{classescount}). Dla zbioru testowego nie jesteœmy w stanie okreœliæ stosunku obu klas ze wzglêdu na brak etykiet. 
\begin{figure}[H]
\centering \includegraphics[scale=0.7]{classes.eps}
\caption{Liczba przyk³adów w ka¿dej z klas}
\label{classescount}
\end{figure}
Jak widaæ, klasy mocno ró¿ni¹ siê licznoœci¹, co negatywnie wp³ywa na poziom trudnoœci ca³ego zagadnienia.

\subsubsection{EDA (Exploratory Data Analysis)}

Eksploracyjne badanie danych jest podstawowym narzêdziem w analizie danych pozwalaj¹cym na szczegó³owy opis, wizualizacjê oraz badanie danych bez koniecznoœci zak³adania hipotez co do tych danych. Jest to w g³ównej mierze metoda oparta o statystykê, a jej wyniki pozwalaj¹ na ukierunkowanie kolejnych badañ na konkretne cechy posiadanych danych. 
\newline W tablicy \ref{traindescribe} zestawiono dane statystyczne opisuj¹ce ka¿d¹ z cech (kolumny kolejno: nazwa cechy, œrednia, odchylenie standardowe, minimum, kwantyl 25\%, mediana, kwantyl 75\%, maksimum) dla zbioru treningowego.
...
Z tabeli \ref{traindescribe} wynika, ¿e odchylenie standardowe du¿ej liczby cech jest znaczne. Kolejn¹ obserwacj¹ jest podobny zakres wartoœci danych ka¿dej cechy. Wszystkie wartoœci ze zbioru treningowego mieszcz¹ siê w zakresie $[-91, 75]$, co jest zaskakuj¹co ma³ym przedzia³em dla tak du¿ej liczby cech. Za to œrednie wartoœci poszczególnych atrybutów s¹ bardzo zró¿nicowane i wynosz¹ od $-16.5$ do $24.5$. Warto zwróciæ uwagê na zmienn¹ o numerze $68$, dla której minimum wynosi 5.018893, a maksimum 5.0469 (odchylenie standardowe: 0.007186). Jest to zmienna o cecha o najmniejszym rozstrzale wartoœci.
\newline 

Istotny mo¿e byæ równie¿ rozk³ad wartoœci poszczególnych cech w zale¿noœci od klasy.
Rysunek \ref{featuresDistribution25} przedstawia wizualizacjê tych¿e rozk³adów dla pierwszych 25 cech (w zbiorze treningowym). Wykresy dla wszystkich 200 kolumn znajduj¹ siê w za³¹czonym pliku \textit{AllFeaturesDistribution.png}.

\begin{figure}[H]
\centering 
\includegraphics[width = 472pt]{feature_distribution.eps}
\caption{Rozk³ad wartoœci pierwszych 25 cech w podziale na klasy (dotyczy zbioru treningowego)}
\label{featuresDistribution25}
\end{figure}

Wnioskuj¹c z omawianych wizualizacji, rozk³ad wartoœci jest zbli¿ony w obu klasach dla wszystkich rozwa¿anych kolumn. Wykresy niemal¿e pokrywaj¹ siê. Jednak czêœæ cech (np. dla var0, var2, var6 itd.) charakteryzuje siê delikatnymi rozbie¿noœciami. Mo¿na wnioskowaæ, ¿e bêd¹ one szczególnie istotne w procesie klasyfikacji.

Warto te¿ porównaæ rozk³ad cech zbioru treningowego i testowego. Wizualizacja dla pierwszych 25 cech zosta³a przedstawiona na rysunku \ref{features_distribution_train_vs_test}.

\begin{figure}[H]
\centering 
\includegraphics[width = 472pt]{feature_distribution_train_vs_test.eps}
\caption{Rozk³ad wartoœci pierwszych 25 cech w zbiorze treningowym i testowym}
\label{features_distribution_train_vs_test}
\end{figure}

Jak mo¿na zauwa¿yæ, zbiory testowy i treningowy s¹ ³udz¹co podobne pod wzglêdem dystrybucji wartoœci poszczególnych zmiennych. Stanowi to du¿¹ zaletê. Prawdopodobnie wykorzystanie informacji z kolumn o najwiêkszych rozbie¿noœciach przy podziale na klasy (w zbiorze treningowym) przyniesie sukces równie¿ w przypadku zbioru testowego.

\subsubsection{PCA (Principal Component Analysis)}
PCA sciœle wi¹¿e sie z metodami wyboru cech i algorytmami które to realizuj¹.
Dane sk³adaj¹ siê z 200 cech liczbowych, dlatego warto sprawdziæ czy wszystkie wprowadzaj¹ now¹ informacjê. Mo¿e siê tak zda¿yæ, ¿e niektóre z tych cech s¹ ze sob¹ skorelowane na tyle, ¿e mo¿na niektóre z nich pomin¹æ w modelu nie trac¹c na jego jakoœci. Ponad to mo¿e siê tak zda¿yæ, ¿e u¿ycie wszystkich dostêpnych cech mo¿e daæ w rezultacie gorsze wyniki ni¿ u¿ycie podzbioru zbioru cech. W celu zbadania, które z dostêpnych cech maj¹ istotny wp³yw na dany problem przeprowadza siê analizê PCA. Celem takiej analizy jest znalezienie zbioru cech, których u¿ycie zmaksymalizuje jakoœæ modelu przy jednoczesnym zminimalizowaniu zbioru cech u¿ytych w danym problemie.
\newline
Na poni¿ej zosta³a zaprezentowana macierz korelacji cech zbioru testowego.
\begin{figure}[H]
\centering 
...
Z powy¿szego wykresu mo¿emy odczytaæ to ¿e cechy nie s¹ ze sob¹ skorelowane lub s¹ w bardzo ma³ym stopniu. \\
\\
\begin{tabular}{llr}
   &   &        Wartoœæ korelacji \\
...
Powy¿sza tabela przedstawiaj¹ca 10 najbardziej skorelowanych cech w zbiorze testowym upewnia nas, ¿e wygl¹d macie¿y korelacji przedstawionej na rysunku \ref{feature_correlation_matrix} to nie b³¹d, a jedynie fakt, ¿e cechy s¹ od siebie niezale¿ne.
Taka informacja nie pozwala nam na odrzuceniu niektórych kolumn, poniewa¿ na wykresie widzimy, ¿e potencjalnie ka¿da cecha wnosi coœ nowego i mo¿e mieæ istotny wp³yw na jakoœæ przysz³ego modelu.
\newline
Przy u¿yciu dostêpnych bibliotek takich jak "sklearn" w jêzyku Python mo¿na wykonaæ redukcjê wymiarowoœci problemu przy pomocy dedytkowanej do tego klasy "PCA". Przy jej pomocy jesteœmy w stanie zredukowaæ praktycznie dowolnie wymiar zadania, wiêkszym lub miejszym kosztem.
Na przyk³ad przy zachowaniu 95\% wariancji dostêpny w bibiotece algorytm redukuje wymiar do 190 cech, co jeszcze bardziej pokazuje, ¿e ka¿da cecha mo¿e mieæ istotny wp³yw na wynik (liniowa zale¿noœæ zachowania wariancji do iloœci cech). Jednak aby siê o tym przekonaæ w 100-u procentach nie zbêdne bêd¹ testy na konkretnych modelach i algorytmach wybranych do rozwi¹zania tego problemu.

\section{Ocena modeli}

Sposób oceny modeli zosta³ narzucony przez organizatorów konkursu. Ze wzglêdu na nierównomierny podzia³ klas, do oceny modeli wykorzystana jest miara AUC, któr¹ mo¿na uzyskaæ poprzez policzenie pola pod krzyw¹ ROC.
